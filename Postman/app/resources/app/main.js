...
const MOVE_DIALOG_MESSAGE = 'Move to Applications Folder?',
      ...
      SUPPORT_LINK = 'http://go.pstmn.io/troubleshoot-could-not-open',
  ...
  function populateInstallationId () {
    appSettings && appSettings.get('installationId', (err, id) => {
      ...
      if (id) {
        ...
        err && pm.logger.warn('main~populateInstallationId: Got installationId:', id, 'from appSettings but with an error', err);
      } else {
        // Logging if id was still not available when no error occurred
        !err && pm.logger.warn('main~populateInstallationId: Failed to get installationId from appSettings but with no error');

        // Logging if error occurs and id was not fetched
        err && pm.logger.warn('main~populateInstallationId: Failed to get installationId from appSettings with an error', err,
